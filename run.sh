...
if [ $SYSTEM_ARCH = "tegra-aarch64" ]; then
	...
	$SUDO docker run --runtime nvidia --env NVIDIA_DRIVER_CAPABILITIES=compute,utility,graphics -d --restart unless-stopped -id --network host \
		--shm-size=8g \
	...
	$SUDO docker run --gpus all -d --restart unless-stopped -id --network=host \
		--shm-size=8g \
		--ulimit memlock=-1 \
